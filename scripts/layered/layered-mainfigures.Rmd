...
This pdf is generated from an R Markdown script. The Markdown generates figures and other results for "Modeling layered non-pharmaceutical interventions against SARS-CoV-2 in the United States with Corvid", available at https://www.medrxiv.org/content/10.1101/2020.04.08.20058487v1.
...
#	cat(scenario, filename, fill=TRUE)
        ...
        if ((!logy & plottype=="symptomatic") |
            (logy & plottype=="cumulative" & triggerday[[scenario]]==90)) {
            legend(ifelse(plottype=="symptomatic","topright","bottomright"), legend=scenarios, fill=pallayers, cex=ifelse(plottype=="symptomatic",0.62,0.75), inset=ifelse(plottype=="symptomatic",0.01,0.06), bg="white")
        }
    }
}
...
#	cat(scenario, filename, fill=TRUE)
    ...
    if ((!logy & plottype=="symptomatic") |
        (logy & plottype=="cumulative" & triggerday[[scenario]]==90)) {
        legend(ifelse(plottype=="symptomatic","topright","bottomright"), legend=scenarios, fill=pallayers, cex=ifelse(plottype=="symptomatic",0.55,0.72), inset=ifelse(plottype=="symptomatic",0.01,0.06), bg="white")
    }
  }
}
plot(x=NA, y=NA, xlab="", ylab="", main="", axes=FALSE, lwd=2, cex.main=1, cex.lab=0.8, ylim=c(ymin,ymax), xlim=c(0,xmax))
...
#	cat(scenario, filename, fill=TRUE)
        ...
        if ((!logy & plottype=="symptomatic") | 
            (logy & plottype=="cumulative")) {
            legend(ifelse(plottype=="symptomatic","topright","bottomright"), legend=scenarios2, fill=pallayers[scenarios2], cex=ifelse(plottype=="symptomatic",0.5,0.7), inset=ifelse(plottype=="symptomatic",0.01,0.06), bg="white")
        }
    }
}

...
```{r table-infectionsbysetting, echo=FALSE}
temp <- data.frame(setting=c("school","work","home","community/neighbors","total"),
                   infected26=c(sum(individuals.seattle[["2.6"]]$SOURCETYPE=="daytime.school",na.rm=TRUE),
           sum(individuals.seattle[["2.6"]]$SOURCETYPE=="daytime.work",na.rm=TRUE),
	   sum(individuals.seattle[["2.6"]]$SOURCETYPE %in% c("daytime.family","nighttime.family"),na.rm=TRUE),
	   sum(individuals.seattle[["2.6"]]$SOURCETYPE %in% c("daytime.comm","daytime.nh","nighttime.comm","nighttime.nh","nighttime.hhcluster"),na.rm=TRUE),
	   sum(!is.na(individuals.seattle[["2.6"]]$SOURCETYPE) & individuals.seattle[["2.6"]]$SOURCETYPE!="seeded")),
                   infected20=c(sum(individuals.seattle[["2"]]$SOURCETYPE=="daytime.school",na.rm=TRUE),
           sum(individuals.seattle[["2"]]$SOURCETYPE=="daytime.work",na.rm=TRUE),
	   sum(individuals.seattle[["2"]]$SOURCETYPE %in% c("daytime.family","nighttime.family"),na.rm=TRUE),
	   sum(individuals.seattle[["2"]]$SOURCETYPE %in% c("daytime.comm","daytime.nh","nighttime.comm","nighttime.nh","nighttime.hhcluster"),na.rm=TRUE),
	   sum(!is.na(individuals.seattle[["2"]]$SOURCETYPE) & individuals.seattle[["2"]]$SOURCETYPE!="seeded")))
...
for (responseday in c(30,90)) {
    for (plottype in c("symptomatic", "cumulative")) {
        ...
        for (scenario in scenarios) {
            filename <- paste("results/Summary-seattle26-layered1-",responseday,"-",ifelse(grepl("school",scenario),"all","none"),"-365-365-",ifelse(grepl("wfh",scenario),0.5,0.0),"-",ifelse(grepl("shelter",scenario),0.3,0.0),"-",ifelse(grepl("iso",scenario),0.5,0.0),"-0-",ifelse(grepl("self",scenario),"treatmentonly","none"),"-",ifelse(grepl("quar",scenario),0.8,0),"-14-1",sep="")
...
for (responseday in c(30,90)) {
  for (plottype in c("symptomatic", "cumulative")) {
    ...
    for (scenario in scenarios) {
	filename <- paste("results/Summary-seattle26-layered1-",responseday,"-",ifelse(grepl("school",scenario),"all","none"),"-",(180-responseday),"-365-",ifelse(grepl("wfh",scenario),0.5,0.0),"-",ifelse(grepl("shelter",scenario),0.3,0.0),"-",ifelse(grepl("iso",scenario),0.5,0.0),"-0-",ifelse(grepl("self",scenario),"treatmentonly","none"),"-",ifelse(grepl("quar",scenario),0.8,0),"-14-1",sep="")
	if (scenario=="no intervention") {
            filename <- "results/Summary-seattle26-layered1-30-none-365-365-0-0-0-0-none-0-14-1"
	}
#	cat(scenario, filename, fill=TRUE)
    ...
    for (plottype in c("symptomatic", "cumulative")) {
        ...
        for (testdelay in c(0,5)) {
            for (scenario in scenarios2) {
                filename <- paste("results/Summary-seattle26-layered1-",responseday,"-",ifelse(grepl("school",scenario),"all","none"),"-",(180-responseday),"-365-",ifelse(grepl("wfh",scenario),0.5,0.0),"-",ifelse(grepl("shelter",scenario),0.3,0.0),"-",ifelse(grepl("iso",scenario),0.5,0.0),"-",testdelay,"-",ifelse(grepl("self",scenario),"treatmentonly","none"),"-",ifelse(grepl("quar",scenario),0.8,0),"-14-1",sep="")
                if (scenario=="no intervention") {
                    filename <- "results/Summary-seattle26-layered1-30-none-365-365-0-0-0-0-none-0-14-1"
                }
#	cat(scenario, filename, fill=TRUE)
...
for (responseday in c(90)) {
    for (scenario in scenarios) {
	filename <- paste("results/Individuals-seattle26-layered1-",responseday,"-",ifelse(grepl("school",scenario),"all","none"),"-365-365-",ifelse(grepl("wfh",scenario),0.5,0.0),"-",ifelse(grepl("shelter",scenario),0.3,0.0),"-",ifelse(grepl("iso",scenario),0.5,0.0),"-0-",ifelse(grepl("self",scenario),"treatmentonly","none"),"-",ifelse(grepl("quar",scenario),0.8,0),"-14-1",sep="")
	if (scenario=="no intervention") {
